...
Sultan Backend can be embedded directly inside your Android app as a native JNI library (`libsultan_android.so`). The Android app loads the library and calls JNI methods to start and stop the server. No separate process or Termux is required.
...
### 3. Start the Server from Your Application
...
class MyApplication : Application() {
    ...
    override fun onCreate() {
        ...
        // Use the app's private files directory for the database
...
Once the server is running, make requests to `http://localhost:8721`:
