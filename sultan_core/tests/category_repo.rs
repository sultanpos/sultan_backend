...
use sultan_core::{
    ...
    testing::storage::category,
};

async fn create_sqlite_category_repo() -> (RepoCtx<DatabaseConnection>, impl CategoryRepository) {
    let repo_ctx = common::init_sqlite_repo_ctx().await;
    (
        repo_ctx,
        sultan_core::storage::sqlite::category::SqliteCategoryRepository::new(),
    )
}
...
async fn test_create_category() {
    let (ctx, repo) = create_sqlite_category_repo().await;
