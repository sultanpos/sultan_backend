...
use sultan_core::domain::model::supplier::{SupplierCreate, SupplierFilter, SupplierUpdate};
use sultan_core::snowflake::SnowflakeGenerator;
use sultan_core::storage::sqlite::supplier::SqliteSupplierRepository;
use sultan_core::storage::supplier_repo::SupplierRepository;
...
fn default_pagination() -> PaginationOptions {
    PaginationOptions::new(1, 100, None)
}

fn default_filter() -> SupplierFilter {
    SupplierFilter {
        name: None,
        code: None,
        phone: None,
...
async fn test_supplier_repo_integration() {
    ...
    let repo = SqliteSupplierRepository::new(pool);
    let ctx = Context::new();
    ...
    // Test Get All
    let suppliers = repo
        .get_all(&ctx, &default_filter(), &default_pagination())
        .await
...
async fn test_filter_by_name() {
    ...
    let suppliers = repo
        .get_all(&ctx, &filter, &default_pagination())
        .await
